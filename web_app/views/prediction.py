...
def build_input_form(features_info):
    """Dynamically build input form from feature info dict."""
    data = {}
    cols = st.columns(2)
        ...
        with cols[i % 2]:
            if info["type"] == "number":
                data[key] = st.number_input(
                    info["label"], min_value=float(info["min"]),
                    max_value=float(info["max"]), step=float(info["step"]),
                    value=float(info["min"]), key=f"pred_{key}"
                )
            elif info["type"] == "select":
                options = info["options"]
                data[key] = st.selectbox(info["label"], options, key=f"pred_{key}")
    return data
